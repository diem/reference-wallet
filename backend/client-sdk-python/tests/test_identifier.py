...
def test_encode_addr_success():
    ...
    enocded_addr = identifier.encode_account(
        test_onchain_address, zero_sub_address, "lbr"
    )
    assert enocded_addr == enocded_addr_with_none_subaddr

    # test with some subaddress
    enocded_addr = identifier.encode_account(
        test_onchain_address, test_sub_address, "lbr"
    )
...
def test_encode_addr_fail():
    ...
    with pytest.raises(InvalidAccountAddressError):
        identifier.encode_account(
            test_onchain_address + "ff", test_sub_address[:-2], "lbr"
        )
...
def test_decode_addr_fail():
    ...
    invalid_version_encoded_address = (
    ...
    invalid_checksum_encoded_address = "lbr1p7ujcndcl7nudzwt8fglhx6wxn08kgs5tm6mz4usw5p72p"  # change last char from t to p
    with pytest.raises(ValueError):
        identifier.decode_account(invalid_checksum_encoded_address, "lbr")

    # fail to decode mixed case per BIP 173
    mixedcase_encoded_address = (
    ...
    short_encoded_address = (
        "lbr1p7ujcndcl7nudzwt8fglhx6wxnvqqqqqqqqqqqqelu3xv"  # sample 23 bytes encoded
    )
    with pytest.raises(ValueError):
    ...
    invalid_separator_encoded_address = (
...
def test_intent_identifier():
    ...
    assert intent_id == "diem://%s?c=%s&am=%d" % (
        enocded_addr_with_none_subaddr,
        "Coin1",
        123,
    )
...
def test_intent_identifier_decode_errors():
    ...
    with pytest.raises(identifier.InvalidIntentIdentifierError):
        identifier.decode_intent(
            "diem://%s?c=Coin1&am=str" % (enocded_addr_with_none_subaddr), "lbr"
        )
    ...
    with pytest.raises(identifier.InvalidIntentIdentifierError):
        identifier.decode_intent(
            "diem://%s?c=Coin1&am=2&am=3" % (enocded_addr_with_none_subaddr), "lbr"
        )
    ...
    with pytest.raises(identifier.InvalidIntentIdentifierError):
        identifier.decode_intent(
            "hello://%s?am=2&c=Coin1" % (enocded_addr_with_none_subaddr), "lbr"
        )
