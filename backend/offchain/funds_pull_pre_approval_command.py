...
class FundsPullPreApprovalCommand(Command):
    ...
    def validate_is_initial(self) -> None:
        if not state_machine.is_valid_initial_status(
            self.funds_pull_pre_approval.status
        ):
    ...
    def validate_write_once_fields(self, prior: "FundsPullPreApprovalCommand") -> None:
        validate_write_once_fields(
            ...
            self.funds_pull_pre_approval,
            prior.funds_pull_pre_approval,
        )
    ...
    def new_request(self) -> CommandRequestObject:
        return new_funds_pull_pre_approval_request(
            funds_pull_pre_approval=self.funds_pull_pre_approval, cid=self.cid
        )
