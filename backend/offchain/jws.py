...
The `serialize` and `deserialize` functions handle JWS message with the following requirements:
  ...
  1. Protected header must constains `{"alg": "EdDSA"}`
...
def serialize(
    ...
    return encode(to_json(obj), sign)
...
def decode(
    ...
    if (
        not isinstance(protected_headers, dict)
        or protected_headers.get("alg") != DIEM_ALG
    ):
