...
from diem import diem_types, identifier, txnmetadata
...
class PaymentCommand(Command):
    ...
    def validate_state_trigger_actor(self) -> None:
        if self.inbound and self.counterparty_actor() != self.state_trigger_actor():
            raise command_error(
                ErrorCode.invalid_command_producer,
                f"{self.counterparty_actor()} should not produce {self}",
            )
    ...
    def validate_is_initial(self) -> None:
        if not self.is_initial():
            msg = f"{self} is not initial object, or could not find payment object by reference id: {self.reference_id()}"
    ...
    def validate_transition(self, prior: "PaymentCommand") -> None:
        if not self.is_valid_transition(prior):
            raise command_error(
                ErrorCode.invalid_transition, f"can not transit from {prior} to {self}"
            )
    ...
    def new_command(
        ...
        return PaymentCommand(
            my_actor_address=self.my_actor_address, payment=new_payment, inbound=inbound
        )
    ...
    def opponent_actor_obj(self) -> PaymentActorObject:
        warnings.warn(
            "`opponent_actor_obj` is deprecated, use `counterparty_actor_obj`"
        )
    ...
    def state(self) -> State[PaymentObject]:
        ...
        try:
            ...
            fields = ", ".join(
                map(lambda f: f"command.payment.{f}", e.match_result.mismatched_fields)
            )
    ...
    def is_valid_transition(self, prior: "PaymentCommand") -> bool:
        ...
        return payment_states.is_valid_transition(
            prior.state(), self.state(), self.payment
        )
    ...
    def is_abort(self) -> bool:
        return (
            self.payment.sender.status.status == Status.abort
            or self.payment.receiver.status.status == Status.abort
        )
    ...
    def travel_rule_metadata_and_sig_msg(self, hrp: str) -> typing.Tuple[bytes, bytes]:
        return txnmetadata.travel_rule(
            ...
            self.sender_account_address(hrp),
            self.payment.action.amount,
        )
