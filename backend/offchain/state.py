...
class Machine(typing.Generic[T]):
    ...
    def match_state(self, event_data: T) -> State[T]:
        ...
        if not ret:
            raise NoStateMatchedError(
                f"could not find state matches given event data({event_data})"
            )
        if len(ret) > 1:
            raise TooManyStatesMatchedError(
                f"found multiple states({ret}) match given event data({event_data})"
            )
        return ret[0]
    ...
    def match_states(self, event_data: T) -> typing.List[State[T]]:
        return [
            state
            for state, match in self.match_states_and_results(event_data)
            if match.success
        ]

    def match_states_and_results(
        self, event_data: T
    ) -> typing.List[typing.Tuple[State[T], MatchResult]]:
...
def require(
    *args: Condition[T], validation: typing.Optional[Condition[T]] = None
) -> Require[T]:
