...
def test_state_and_match_result():
    ...
    assert d.match(o) == MatchResult(
        success=False, matched_fields=["b"], mismatched_fields=["c"]
    )
    assert d.match(
        Object(a="hello", b=Object(a="world"), c=Object("!"))
    ) == MatchResult(success=True, matched_fields=["b", "c"])
    assert d.match(Object(a="hello")) == MatchResult(
        success=False, mismatched_fields=["b", "c"]
    )
...
def test_require_with_validation():
    ...
    assert st.match(Object()) == MatchResult(
        success=False, matched_fields=[], mismatched_fields=["a", "b"]
    )
    assert st.match(Object(a="hello")) == MatchResult(
        success=False, matched_fields=["a"], mismatched_fields=["b"]
    )
...
def test_match_states_by_context_object_field_value():
    ...
    e = State(
        id="e", require=require(OneOfValues(path="b.a", value1="bond", value2="gurki"))
    )
    ...
    assert m.match_states(Object(a="hello", c=Object())) == [a]
    assert m.match_states(Object(a="dog", b=Object(a="gurki"))) == [e]
    assert m.match_states(Object(a="dog", b=Object(a="bond"))) == [e]
